...
public class Upload extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		List<Stream> s1 = OfyService.ofy().load().type(Stream.class).list();
		for ( Stream str : s1 ) {
			if ( str.id.equals(streamId) ) {
				str.numPics++;
				str.lastAdded = new Date();
//				System.out.println("Date " + str.lastAdded);
 				OfyService.ofy().save().entity(str).now();
			}
		}
		
		res.sendRedirect("/show_stream.jsp?streamId=" + streamId + "&"
